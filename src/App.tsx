...
export default function App() {
  ...
  const [sidebarMode, setSidebarMode] = createSignal<"edit" | "worksheet">(
    "worksheet",
  );
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      // Extract title from markdown header content (first line without # prefix)
      const headerContent = score().metadata.headerContent || '';
      const firstLine = headerContent.split('\n')[0] || '';
      const title = firstLine.replace(/^#+\s*/, '').trim() || "worksheet";
    ...
    <div class="app paper-texture">
      ...
      <header class="app-header">
        ...
        <div class="header-center">
          <span class="app-title">Maestro Blocks</span>
        </div>
      ...
      {/* Main Layout */}
      <div class="app-body">
        <Show when={sidebarMode() === "worksheet"} fallback={<Sidebar />}>
      ...
      <footer class="status-bar">
        <div class="status-left">
          <span class="status-item">
            ...
            {score().sections.length} section
            {score().sections.length !== 1 ? "s" : ""}
          </span>
          <span class="status-divider"></span>
          <span class="status-item">
            {score().keySignature.fifths === 0
              ? "C Major"
              : score().keySignature.fifths > 0
                ? `${score().keySignature.fifths} sharp${score().keySignature.fifths !== 1 ? "s" : ""}`
        ...
        <div class="status-right">
          <span class="status-item muted">VexFlow rendering</span>
        </div>
      </footer>
    </div>
  );
